...
def optimize(animal_count, nutrient_req_table, crop_nutrient_table, methane_eqn):
    """
    ...
    nel = m.addVar(lb=0, name="nel") # kg/cow/d
    me = m.addVar(lb=0, name="me") # kg/cow/d
    ...
    for c in crops:
        ...
        m.addConstr(feed_on_farm[c] >= min_val, name=f"min_crop_{c}")
        m.addConstr(feed_on_farm[c] <= max_val, name=f"max_crop_{c}")

    # PART 3: CALC ENTERIC METHANE
    m.addConstr(
        nel ==
        (
    ...
    results_data = {
        "Variable": ["methane (g/cow/d)", "dmi (kg/cow/d)", "dm (%)", "NEL (Mcal/kg)", "CP (% in DM)", "NDF (% in DM)", "STARCH (% in DM)", "FAT (% in DM)", "TFA (% in DM)", "DNDF (% in DM)"], 
        "Value": [methane.X*1000, dmi.X, dmi.X/total_as_fed*100, nel.X/dmi.X] + [nutrient_vars[n].X/dmi.X*100 for n in other_nutrients if n != 'NEL'] + [tfa.X/dmi.X*100, dndf.X/dmi.X*100]
    }
    results_df = pd.DataFrame(results_data)

    return results_df.round(2), feed_df.round(2)

def group_and_opt(group_num, criteria, cow_df, crop_nutrient_table, DM_vary=0.001, NEL_vary=0.001, methane_eqn='NASEM'):
    """
    ...
    if group_num == 1:
        ...
        if criteria == 'dim':
            ...
            raise ValueError("Invalid criteria. Use 'dim' or 'nel'.")
    elif group_num == 3:
    ...
    results_df1, feed_df1 = optimize(len(group1_df), nutrient_req_table1, crop_nutrient_table, methane_eqn=methane_eqn)
    ...
    if group_num >= 2:
        ...
        results_df2, feed_df2 = optimize(len(group2_df), nutrient_req_table2, crop_nutrient_table, methane_eqn=methane_eqn)
        ...
        if group_num == 3: # Group 3 (if applicable)
            ...
            results_df3, feed_df3 = optimize(len(group3_df), nutrient_req_table3, crop_nutrient_table, methane_eqn=methane_eqn)
...
group_and_opt(2,'nel', cow_df, crop_nutrient_table, DM_vary, NEL_vary, methane_eqn='Ellis')
