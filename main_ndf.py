...
def group_and_opt(group_num, criteria, cow_df, crop_nutrient_table, DM_vary, NEL_vary, NDF_vary, methane_eqn='NASEM', obj='cost'):
    """
    ...
    if group_num == 1:
        ...
        nutrient_req_table1 = util.construct_nutritional_req_table(DM_baseline=group1_df['DMI'].mean(), NEL_baseline=np.percentile(group1_df['NEL'], NEL_req_percentile), NDF_baseline=group1_df['BW'].mean()*0.013, DM_vary=DM_vary, NEL_vary=NEL_vary, NDF_vary=NDF_vary)
        ...
        if criteria == 'bw':
            group1_df, group2_df = util.two_group_by_bw(cow_df)
            print("Group 1 stats:", util.get_descriptive_stats(group1_df))
            nutrient_req_table1 = util.construct_nutritional_req_table(DM_baseline=group1_df['DMI'].mean(), NEL_baseline=np.percentile(group1_df['NEL'], NEL_req_percentile), NDF_baseline=group1_df['BW'].mean()*0.013, DM_vary=DM_vary, NEL_vary=NEL_vary, NDF_vary=NDF_vary)
            print("Nutritional requirement table for Group 1:")
            print(nutrient_req_table1)
            ...
            nutrient_req_table2 = util.construct_nutritional_req_table(DM_baseline=group2_df['DMI'].mean(), NEL_baseline=np.percentile(group2_df['NEL'], NEL_req_percentile), NDF_baseline=group2_df['BW'].mean()*0.013, DM_vary=DM_vary, NEL_vary=NEL_vary, NDF_vary=NDF_vary)
            print("Nutritional requirement table for Group 2:")
            print(nutrient_req_table2)
...
NDF_vary = 0.1
cow_df = util.get_cow_raw_data('./data/cow_raw_data.csv') 
group_and_opt(2,'bw', cow_df, crop_nutrient_table, DM_vary, NEL_vary, NDF_vary = NDF_vary, methane_eqn='NASEM', obj = 'methane')
###test group by NDF###
